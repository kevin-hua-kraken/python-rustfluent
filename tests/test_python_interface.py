...
def test_get_parse_errors_no_errors():
    """Test that get_parse_errors() returns empty list when no parse errors."""
    bundle = fluent.Bundle("en", [data_dir / "en.ftl"])
    parse_errors = bundle.get_parse_errors()
...
def test_get_required_variables_no_variables():
    """Test that messages without variables return empty list."""
    bundle = fluent.Bundle("en", [data_dir / "variables.ftl"])
    variables = bundle.get_required_variables("static-message")
...
def test_get_required_variables_from_existing_message():
    """Test get_required_variables with en.ftl messages."""
    bundle = fluent.Bundle("en", [data_dir / "en.ftl"])

    # hello-user has $user variable
    variables = bundle.get_required_variables("hello-user")
    assert variables == ["user"]

    # apples has $numberOfApples variable
    variables = bundle.get_required_variables("apples")
    assert variables == ["numberOfApples"]

    # hello-world has no variables
    variables = bundle.get_required_variables("hello-world")
    assert variables == []
...
def test_get_translation_errors_all_variables_provided():
    """Test that no errors are reported when all variables are provided."""
    bundle = fluent.Bundle("en", [data_dir / "variables.ftl"])
    errors = []

    result = bundle.get_translation(
        "user-info", variables={"username": "Alice", "count": 5}, errors=errors
    )
...
# ==============================================================================
# FormatError Class Tests
# ==============================================================================


def test_format_error_repr():
    """Test FormatError __repr__ method."""
    bundle = fluent.Bundle("en", [data_dir / "variables.ftl"])
    errors = []

    # Generate a FormatError
    bundle.get_translation("greeting", errors=errors)
    ...
    # Test __repr__
    repr_str = repr(error)
    assert "FormatError" in repr_str
    assert "error_type=" in repr_str
    assert "message=" in repr_str


def test_format_error_str():
    """Test FormatError __str__ method."""
    bundle = fluent.Bundle("en", [data_dir / "variables.ftl"])
    errors = []
    ...
    # Test __str__
    str_repr = str(error)
    assert "MissingVariable" in str_repr
    assert "Unknown external: name" in str_repr


def test_format_error_attributes():
    """Test that FormatError has all expected attributes."""
    bundle = fluent.Bundle("en", [data_dir / "variables.ftl"])
    errors = []

    # Generate a MissingVariable error
    bundle.get_translation("greeting", errors=errors)
    ...
    # Verify values for MissingVariable error
    assert error.error_type == "MissingVariable"
    assert error.message_id == "greeting"
    assert error.variable_name == "name"


def test_format_error_invalid_type_has_type_info():
    """Test that InvalidVariableType errors include type information."""
    bundle = fluent.Bundle("en", [data_dir / "variables.ftl"])
    errors = []

    # Pass wrong type
    bundle.get_translation("greeting", variables={"name": {"not": "a string"}}, errors=errors)

    error = errors[0]
    assert error.error_type == "InvalidVariableType"
    assert error.expected_type == "str|int|date"
    assert error.actual_type is not None
    assert "dict" in error.actual_type.lower() or "PyDict" in error.actual_type
